    ...
    <section aria-labelledby="work-heading">
      <div className="mb-2 flex items-baseline justify-between border-b border-line pb-3">
        ...
        <span className="text-[12px] tabular-nums text-ink-faint">
          {String(cases.length).padStart(2, "0")} / 07
        </span>
      </div>
      ...
      <ul>
        {cases.map((c) => (
          <li key={c.slug} className="border-b border-line">
            <Link
              href={`/work/${c.slug}`}
              className="group relative -mx-4 grid grid-cols-[1fr_auto] items-baseline gap-4 rounded-md px-4 py-6 transition-colors duration-200 ease-[cubic-bezier(0.4,0,0.2,1)] hover:bg-white/[0.03] sm:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,1.2fr)_auto] sm:gap-10 sm:py-7"
            >
