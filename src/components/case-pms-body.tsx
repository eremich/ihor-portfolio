...
export function Block({ kicker, id, children }: { kicker: string; id?: string; children: React.ReactNode }) {
  return (
  ...
  return (
    <figure className="mt-10">
      ...
      >
        ...
        {/* Very wide diagrams keep a readable size and scroll sideways instead of shrinking. */}
        <div className={wide ? "overflow-x-auto" : undefined}>
          <img src={board.src} alt={board.alt} loading="lazy" className={`h-auto rounded-md ${wide ? "w-[1700px] max-w-none" : "w-full"}`} />
        </div>
      </a>
...
export function Lead({ title, className = "", children }: { title: string; className?: string; children: React.ReactNode }) {
  return (
